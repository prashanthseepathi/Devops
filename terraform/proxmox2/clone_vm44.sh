...
NEW_VM_NAME="sandbox-terraform2"
...
template_vm_response=$(curl -k -s -X GET "${API_URL}/nodes/${TARGET_NODE}/qemu" \
    --header "Authorization: PVEAPIToken=${TOKEN_ID}=${TOKEN_SECRET}")

log "Template VM response: $template_vm_response"

template_vm_id=$(echo "$template_vm_response" | jq -r '.data[] | select(.name == "'"${TEMPLATE_NAME}"'") | .vmid')
...
log "Cloning initiated successfully. UPID: $upid"

# Wait for the cloning process to complete (optional)
log "Waiting for cloning to complete..."
while true; do
  task_status=$(curl -k -s -X GET "${API_URL}/nodes/${TARGET_NODE}/tasks/${upid}/status" \
      --header "Authorization: PVEAPIToken=${TOKEN_ID}=${TOKEN_SECRET}" | jq -r '.data.status')
  
  if [ "$task_status" == "stopped" ]; then
    log "Cloning completed."
    break
  elif [ "$task_status" == "error" ]; then
    log "Error during cloning process. Check Proxmox logs for details."
    exit 1
  else
...
start_response=$(curl -k -s -X POST "${API_URL}/nodes/${TARGET_NODE}/qemu/${available_vm_id}/status/start" \
    --header "Authorization: PVEAPIToken=${TOKEN_ID}=${TOKEN_SECRET}")

log "Start VM response: $start_response"

log "Successfully cloned and started VM with ID: $available_vm_id"
